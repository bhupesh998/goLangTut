...
func CreateOrderItem() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if insertErr != nil {
			msg := fmt.Sprintf("Unable to Insert Order Item ")
...
func UpdateOrderItem() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		result , updateErr := orderItemCollection.UpdateOne(ctx, filter, bson.D{
			{"$set" ,updateObj},
		}, &opt)

		if updateErr != nil {
			msg := fmt.Sprintf("Unable to Update Order Item ")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func GetOrderItemsByOrder() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if err != nil{
			msg := fmt.Sprintf("Order Items not found for the order")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func ItemsByOrder(id string)(OrderItems []primitive.M, err error){
	...
	lookupStage := bson.D{{"$lookup", bson.D{{"from", "food"}, {"localFeild", "food_id"}, {"ForeignFeild", "food_id"}, {"as", "food"}}}}
	unwindStage := bson.D{{ "$unwind", bson.D{{"path", "$food"}, {"preserveNullAndEmptyArrays", true}}}}


	lookupOrderStage := bson.D{{"$lookup", bson.D{{"from", "order"}, {"localFeild", "order_id"}, {"ForeignFeild", "order_id"}, {"as", "order"}}}} 
	unwindOrderStage :=  bson.D{{ "$unwind", bson.D{{"path", "$order"}, {"preserveNullAndEmptyArrays", true}}}}

	lookupTableStage := bson.D{{"$lookup", bson.D{{"from", "table"}, {"localFeild", "order.table_id"}, {"ForeignFeild", "table_id"}, {"as", "table"}}}} 
