...
func CreateInvoice() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if insertErr != nil {
			msg := fmt.Sprintf("Unable to Insert Invoice")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func UpdateInvoice() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		result , updateErr := invoiceCollection.UpdateOne(ctx, filter, bson.D{{"$set" ,updateObj},}, &opt)

		if updateErr != nil {
			msg := fmt.Sprintf("Unable to Update Invoice ")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
